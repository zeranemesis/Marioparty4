...
& "$vcPath\bin\Hostx64\x64\cl.exe" /nologo /std:c++20 /EHsc /W4 /WX /O2 `
    ...
    "/Fe$testOutput\netplay_state_test.exe" "/Fo$testOutput\netplay_state_test.obj" `
    "$projectPath\tools\tests\netplay_state_test.cpp" /link `
...
& "$testOutput\netplay_state_test.exe"
if ($LASTEXITCODE -ne 0) { throw "Canonical state tests failed: $LASTEXITCODE" }
