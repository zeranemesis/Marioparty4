...
$testOutput = Join-Path $projectPath 'build\rollback-effect-tests'
New-Item -ItemType Directory -Force -Path $testOutput | Out-Null
& "$vcPath\bin\Hostx64\x64\cl.exe" /nologo /std:c++20 /EHsc /W4 /O2 `
    "/I$projectPath\include" "/I$vcPath\include" `
    "/I$sdkPath\Include\$sdkVersion\ucrt" "/I$sdkPath\Include\$sdkVersion\um" `
    "/I$sdkPath\Include\$sdkVersion\shared" `
    "/Fe$testOutput\rollback_effects_test.exe" "/Fo$testOutput\\" `
    "$projectPath\tools\tests\rollback_effects_test.cpp" /link `
    "/LIBPATH:$vcPath\lib\x64" "/LIBPATH:$sdkPath\Lib\$sdkVersion\ucrt\x64" `
    "/LIBPATH:$sdkPath\Lib\$sdkVersion\um\x64"
if ($LASTEXITCODE -ne 0) { throw "Rollback effect journal compilation failed: $LASTEXITCODE" }
& "$testOutput\rollback_effects_test.exe"
if ($LASTEXITCODE -ne 0) { throw "Rollback effect journal tests failed: $LASTEXITCODE" }
