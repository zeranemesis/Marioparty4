...
{
    // Exercise failures before prediction and at every stage of a replay.
    enum Fault { SaveFalse, SaveThrow, LoadFalse, LoadThrow, SimulateThrow, None };
    for (const auto fault : { SaveFalse, SaveThrow, LoadFalse, LoadThrow, SimulateThrow, None }) {
        for (const bool replay : { false, true }) {
            if (!replay && (fault == LoadFalse || fault == LoadThrow)) continue;
            std::uint32_t state = 0;
            unsigned calls = 0;
            bool armed = false;
            Callbacks callbacks;
            callbacks.saveState = [&](void* dest, std::size_t size) {
                ++calls;
                if (armed && fault == SaveThrow) throw std::runtime_error("save");
                if (armed && fault == SaveFalse) return false;
                std::memcpy(dest, &state, size);
                return true;
            };
            callbacks.loadState = [&](const void* src, std::size_t size) {
                ++calls;
                CHECK(PartyBoard_RollbackIsResimulating());
                if (armed && fault == LoadThrow) throw std::runtime_error("load");
                if (armed && fault == LoadFalse) return false;
                std::memcpy(&state, src, size);
                return true;
            };
            callbacks.simulateFrame = [&](std::uint32_t, const auto& inputs) {
                ++calls;
                state += 1 + inputs[0].buttons;
                if (armed) {
                    CHECK(PartyBoard_RollbackIsResimulating() == replay);
                    // A renderer/audio thread must not inherit the replay flag.
                    bool otherThreadReplay = true;
                    std::thread worker([&] { otherThreadReplay = PartyBoard_RollbackIsResimulating(); });
                    worker.join();
                    CHECK(!otherThreadReplay);
                }
                if (armed && fault == SimulateThrow) throw std::runtime_error("simulate");
            };
            Session engine({ 1, 12, sizeof(state) }, callbacks);
            if (replay) {
                CHECK(engine.advance());
                PartyBoardRollbackInput corrected {};
                corrected.buttons = 7;
                CHECK(engine.submitInput(0, 0, corrected));
            }
            armed = true;
            CHECK((replay ? engine.reconcile() : engine.advance()) == (fault == None));
            CHECK(engine.healthy() == (fault == None));
            CHECK(!PartyBoard_RollbackIsResimulating());
            if (fault == None) {
                CHECK(state == (replay ? 8u : 1u));
            } else {
                const auto stoppedCalls = calls;
                CHECK(!engine.advance());
                CHECK(!engine.reconcile());
                CHECK(!engine.submitInput(0, 0, {}));
                CHECK(calls == stoppedCalls);
                CHECK(engine.currentFrame() == (replay ? 1u : 0u));
                // Recovery requires an explicit game-state reset, not a retry.
                state = 0;
                armed = false;
                engine.reset();
                CHECK(engine.advance());
...
static void snapshotRing()
{
    // Exercise the oldest restorable frame exactly at the configured boundary,
    // through multiple ring wraps and reset. Full states must only allocate
    // window+1 slots, independently of the 256-entry controller history.
    for (const std::size_t window : {1u, 12u, 255u}) {
        State expected, observed;
        auto reference = session(expected, 4, window);
        auto delayed = session(observed, 4, window);
        CHECK(delayed->snapshotStorageBytes() == (window + 1) * sizeof(State));
        for (unsigned pass = 0; pass < 2; ++pass) {
            expected = {}; observed = {};
            reference->reset(); delayed->reset();
            for (std::uint32_t frame = 0; frame < 800; ++frame) {
                for (std::size_t player = 0; player < 4; ++player) {
                    CHECK(reference->submitInput(player, frame, input(frame, player)));
                    if (frame >= window)
                        CHECK(delayed->submitInput(player, frame - static_cast<unsigned>(window),
                            input(frame - static_cast<unsigned>(window), player)));
                }
                CHECK(reference->advance());
                CHECK(delayed->advance());
            }
            for (std::uint32_t frame = 800 - static_cast<unsigned>(window); frame < 800; ++frame)
                for (std::size_t player = 0; player < 4; ++player)
                    CHECK(delayed->submitInput(player, frame, input(frame, player)));
            CHECK(delayed->reconcile());
            CHECK(delayed->healthy());
            CHECK(delayed->stats().maximumRollback == window);
