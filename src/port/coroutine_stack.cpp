...
// libco owns the stack-pointer slot at the base and the two return addresses it
// pushes near the top (co_derive reserves 32 bytes then pushes two words). The
// pattern must not touch either, or libco breaks.
constexpr std::size_t kReservedAtBase = 16;
...
std::size_t scanPeak(const Entry &entry)
{
    if (!entry.armed || !entry.usable) return 0;
    if (entry.usableBytes <= kReservedAtBase + kReservedAtTop) return 0;
    ...
    for (std::size_t offset = kReservedAtBase; offset + sizeof(std::uint64_t) <= last;
        ...
        if (word != kPattern) return entry.usableBytes - offset;
    }
    return 0;
}
...
// Caller must hold gMutex.
void retireLocked(const Entry &entry, std::size_t peak)
{
    if (peak == 0 || !entry.entryPoint) return;
    Retired *spare = nullptr;
    for (auto &slot : gRetired) {
        if (slot.used && slot.entryPoint == entry.entryPoint) {
            if (peak > slot.peakBytes) slot.peakBytes = peak;
...
extern "C" void PartyBoard_CoroutineStackFree(void *pointer)
{
    ...
    void *reservation = nullptr;
    {
        ...
        if (entry) {
            retireLocked(*entry, scanPeak(*entry));
...
extern "C" void PartyBoard_CoroutineStackRetire(const void *stack)
{
    ...
    std::lock_guard<std::mutex> guard(gMutex);
    Entry *entry = findEntryLocked(stack);
    if (entry) retireLocked(*entry, scanPeak(*entry));
}
...
extern "C" u32 PartyBoard_CoroutineStackSummary(char *out, size_t outSize)
{
    ...
    for (std::uint32_t index = 0; index < highest && index < kMaxTracked; ++index) {
        ...
        const std::size_t peak = scanPeak(entry);
        if (!peak || room() == 0) continue;
        describeEntryPoint(entry.entryPoint, name, sizeof(name));
        const double ratio = entry.usableBytes
            ? 100.0 * static_cast<double>(peak) / static_cast<double>(entry.usableBytes) : 0.0;
        advance(std::snprintf(out + written, room(),
            "STACK live process=%s requested=%llu pc_size=%llu peak=%llu headroom=%lld usage=%.1f%%\n",
            name, static_cast<unsigned long long>(entry.requestedBytes / 2),
            static_cast<unsigned long long>(entry.usableBytes),
            static_cast<unsigned long long>(peak),
            static_cast<long long>(entry.usableBytes) - static_cast<long long>(peak), ratio));
    }
    for (const auto &slot : gRetired) {
        ...
        advance(std::snprintf(out + written, room(),
            "STACK retired process=%s requested=%llu pc_size=%llu peak=%llu headroom=%lld "
            "usage=%.1f%% samples=%u\n",
...
extern "C" bool PartyBoard_CoroutineStackRunSelfTest(void)
{
    ...
    // A write at a known depth must be reported as exactly that depth.
