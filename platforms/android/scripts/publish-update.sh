...
#
#   publish-update.sh <signed apk> <abi> <versionCode> <versionName>
#
# Everything goes to one GitHub release, partyboard-android-latest:
#   PartyBoard-android-<abi>-<code>.apk  this build; the 5 newest stay, older ones are removed
#   PartyBoard-android-<abi>.apk         the same file under a name that never changes, to
#                                        install the app the first time
#   android-update.json                  the manifest the app reads, uploaded last so it never
...
if [ "$#" -ne 4 ]; then
    echo "usage: $0 <signed apk> <abi> <versionCode> <versionName>" >&2
    exit 2
fi
...
VERSIONED="PartyBoard-android-$ABI-$CODE.apk"
STABLE="PartyBoard-android-$ABI.apk"
...
PREVIOUS=""
if gh release download "$TAG" --repo "$REPO" --pattern android-update.json --dir "$WORK/previous" 2>/dev/null; then
    PREVIOUS=$(jq -r '.commit // empty' "$WORK/previous/android-update.json")
fi
...
jq -n \
    ...
    > "$WORK/android-update.json"
cat "$WORK/android-update.json"

BODY="Party Board for Android, build $CODE ($COMMIT).

Install: download $STABLE on the phone and open it. Once installed, the app finds the next builds itself (Settings > Interface > Check for Updates).
...
gh release upload "$TAG" --repo "$REPO" --clobber "$WORK/android-update.json"
...
gh release view "$TAG" --repo "$REPO" --json assets --jq '.assets[].name' \
    | { grep -E "^PartyBoard-android-$ABI-[0-9]+\.apk$" || true; } \
    | sed -E 's/.*-([0-9]+)\.apk$/\1 &/' \
    | sort -rn \
    | tail -n +$((KEEP + 1)) \
