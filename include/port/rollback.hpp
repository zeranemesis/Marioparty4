...
struct Callbacks {
    // Callbacks run synchronously. Failure or an exception stops the session;
    // reset() only clears history, so the owner must also reset game state.
    // loadState and replay callbacks observe RollbackIsResimulating() == true.
...
public:
    ...
    bool advance();
    // Live engines can execute the current tick in their existing main loop:
    // prepare captures its boundary and resolves inputs; complete publishes it.
    // Historical corrections are still replayed synchronously by prepare.
    bool prepare(std::array<PartyBoardRollbackInput, kMaxPlayers> &inputs);
    bool complete();
    bool reconcile();

    std::uint32_t currentFrame() const { return mCurrentFrame; }
    // Exclusive boundary: only frames below this value have every input
    // confirmed AND their corrected simulation completed. Read after a
    // successful advance/reconcile, never use packet arrival alone for effects.
...
private:
    ...
    void gatherInputs(std::uint32_t frame,
        std::array<PartyBoardRollbackInput, kMaxPlayers> &inputs);
    bool invokeSimulation(std::uint32_t frame,
    ...
    // Input history is cheap; full game snapshots are not. Retain exactly the
    // rewind window and its current boundary, independently of input history.
    std::vector<SnapshotRecord> mSnapshots;
    std::uint32_t mCurrentFrame = 0;
    std::uint32_t mConfirmedFrame = 0;
