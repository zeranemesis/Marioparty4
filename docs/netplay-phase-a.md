...
DESYNC logs the first divergent hash frame, both hashes/contexts/current RNGs/counters, last committed input and last equal state count. There is no confirmed portable checkpoint. Since canonical hash version 2 the summary also names the first differing subsystem, stderr and the .desync sidecar carry the sixteen per-subsystem verdicts, and the historical local projection is dumped by ordinal/name/value into a separate per-peer report (see NETPLAY_DIAGNOSTICS.md) rather than into stderr. The launcher still exports the sidecar separately from its capped routine log. The two peer reports allow exact field comparison without sending full state over UDP.
