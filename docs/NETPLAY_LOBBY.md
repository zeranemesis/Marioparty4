...
Exécution réelle : arrêt `Stopped:VpnOrVirtualRoute`, interface **NordLynx**.
La route VPN était prioritaire sur Ethernet. Aucune requête de découverte à
la box, aucune ouverture de port et aucune modification du VPN n'ont été faites.
L'utilisateur doit choisir lui-même de désactiver temporairement son VPN pour
reprendre le test de sa connexion directe. Cela ne permet pas de conclure que
la box ou l'opérateur sont incompatibles, ni de diagnostiquer un CGNAT.
