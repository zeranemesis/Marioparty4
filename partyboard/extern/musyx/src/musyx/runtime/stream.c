...
void streamHandle() {
  ...
  for (i = 0; i < synthInfo.voiceNum; ++i, ++si) {
    switch (si->state) {
    case 1:
      if (si->voice >= synthInfo.voiceNum) {
        si->state = 3;
        break;
      }
    ...
    case 2: {
      ...
      if (si->last != cpos) {
        ...
        if (si->state == 2 && !(si->flags & 0x20000) && si->type == 1) {
          ...
          /* Stream data keeps the GameCube's big-endian byte layout.  Reading
           * a host u32 and shifting selects byte 3 on little-endian PCs, not
           * the predictor/scale byte at the start of the ADPCM block. */
          hwSetStreamLoopPS(si->voice, (si->lastPSFromBuffer = *(u8*)si->buffer));
#endif
#else
#if MUSY_TARGET == MUSY_TARGET_DOLPHIN
...
#elif MUSY_TARGET == MUSY_TARGET_PC
          hwSetStreamLoopPS(si->voice, *(u8*)si->buffer);
#endif
#endif
        }
      }
...
#elif MUSY_TARGET == MUSY_TARGET_PC
      streamInfo[i].lastPSFromBuffer = *(u8*)streamInfo[i].buffer;
#endif
...
#else
    if (streamInfo[i].type == 1) {
      if (streamInfo[i].voice < synthInfo.voiceNum) {
#if MUSY_TARGET == MUSY_TARGET_DOLPHIN
        hwSetStreamLoopPS(streamInfo[i].voice,
                          *(u32*)MUSY_CACHED_TO_UNCACHED_ADDR(streamInfo[i].buffer) >> 24);
#elif MUSY_TARGET == MUSY_TARGET_PC
        /* The stream buffer keeps its GameCube byte order on the PC. */
        hwSetStreamLoopPS(streamInfo[i].voice, *(u8*)streamInfo[i].buffer);
#endif
      }
    }
#endif
