...
static bool (*sampleResolver)(u16 sampleId);
...
void sndSetSampleResolver(bool (*resolver)(u16 sampleId)) { sampleResolver = resolver; }
...
static bool dataInsertSDirInternal(SDIR_DATA* sdir, void* smp_data, bool ownsData,
                                   bool ownsSampleData) {
  s32 i;        // r31
  SDIR_DATA* s; // r25
  u16 n;        // r27
  ...
  if (i == dataSmpSDirNum) {
    if (dataSmpSDirNum < 128) {
      ...
      dataSmpSDirs[dataSmpSDirNum].ownsData = ownsData;
      dataSmpSDirs[dataSmpSDirNum].ownsSampleData = ownsSampleData;
      ++dataSmpSDirNum;
      hwEnableIrq();
      return 1;
    } else {
...
bool dataInsertSDir(SDIR_DATA* sdir, void* smp_data) {
  return dataInsertSDirInternal(sdir, smp_data, FALSE, FALSE);
}

bool dataInsertOwnedSDir(SDIR_DATA* sdir, void* smp_data) {
  return dataInsertSDirInternal(sdir, smp_data, TRUE, FALSE);
}

bool dataInsertOwnedSDirAndSamples(SDIR_DATA* sdir, void* smp_data) {
  return dataInsertSDirInternal(sdir, smp_data, TRUE, TRUE);
}
...
bool dataRemoveSDir(struct SDIR_DATA* sdir) {
  ...
  if (index != dataSmpSDirNum) {
    ...
    if (data->id == 0xFFFF) {
      ownsData = dataSmpSDirs[index].ownsData;
      ownsSampleData = dataSmpSDirs[index].ownsSampleData;
      sampleData = dataSmpSDirs[index].base;
#if MUSY_VERSION <= MUSY_VERSION_CHECK(2, 0, 1)
      data = sdir;
...
retry:
  ...
  if (sdir == NULL && !resolverAttempted && sampleResolver != NULL) {
    resolverAttempted = TRUE;
    if (sampleResolver(sid)) {
      goto retry;
    }
  }
    ...
    if ((result = sndBSearch(&key, dataSmpSDirs[i].data, dataSmpSDirs[i].numSmp, sizeof(SDIR_DATA),
      ...
      if (result->ref_cnt != (MUSY_VERSION <= MUSY_VERSION_CHECK(2, 0, 1) ? 0xFFFF : 0)) {
        ...
        if (result->extraData) {
          ...
          newsmp->extraData = result->extraData;
#else
...
void* sndConvert32BitSDIRTo64BitSDIR(void* sdir_int) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    /* The legacy SDIR terminator is only four bytes long; bytes following it
     * already belong to the first sample's auxiliary decoder metadata. */
    if (sdir_inter[i].id == 0xffff) {
      sdir[i].offset = 0;
      sdir[i].addr = NULL;
      memset(&sdir[i].header, 0, sizeof(sdir[i].header));
      sdir[i].extraData = NULL;
      break;
    }
    ...
    sdir[i].extraData = sdir_inter[i].extraData != 0
                            ? (void*)((u8*)sdir_inter + sdir_inter[i].extraData)
                            : NULL;
  }
