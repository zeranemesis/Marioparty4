...
static FILE* seqDiagnosticFile;
extern u8 voiceMusicRunning;
extern u8 voiceFxRunning;
extern u32 salAudioDumpFrameCount;
extern u32 missingSampleDiagnosticCount;
extern u32 startedSampleDiagnosticCount;
extern uint64_t startedSampleIdSum;
extern uint64_t startedSampleOffsetSum;
extern uint64_t startedSampleLengthSum;
extern uint64_t startedSampleVoiceVolumeSum;
extern u32 startedSampleLoopedCount;
extern u32 startedSampleNearEndCount;
extern uint64_t startedSampleLagSum;
extern u32 startedSampleLagMax;
extern uint64_t startedSampleCurrentNoteSum;
extern uint64_t startedSampleCurrentNoteHash;
extern uint64_t startedSampleFinalNoteSum;
extern uint64_t startedSampleFinalNoteHash;
extern uint64_t startedSampleInfoSum;
extern uint64_t startedSamplePlayFrqSum;
extern SYNTHMasterFader synthMasterFader[32];
extern u32 synthMasterFaderActiveFlags;
extern uint64_t salDiagPitchSum;
extern uint64_t salDiagPitchHash;
extern u32 salDiagPitchMin;
extern u32 salDiagPitchMax;
extern u32 pitchStageCount;
extern uint64_t pitchStageBaseSum;
extern uint64_t pitchStageBendSum;
extern uint64_t pitchStageVibratoSum;
extern uint64_t pitchStagePortamentoSum;
extern uint64_t pitchStageAdsrSum;
extern uint64_t pitchStageFinalSum;
extern u32 eventRequestCount;
extern u32 eventAlreadyQueuedCount;
extern u32 eventHandledCount;
extern u32 eventStartCount;
extern u32 eventInactiveCount;
extern u32 eventBlockedCount;
extern u32 eventStaleCount;
extern u32 voiceDiagAllocationRequests;
extern u32 voiceDiagAllocationFree;
extern u32 voiceDiagAllocationReplacement;
extern u32 voiceDiagAllocationSameId;
extern u32 voiceDiagAllocationFailed;
extern u32 voiceDiagMusicPeak;
static u32 seqDiagnosticStarts;
static u32 seqDiagnosticStartFailures;
static uint64_t seqDiagnosticLengthSum;
static uint64_t seqDiagnosticVelocitySum;
static uint64_t seqDiagnosticKeySum;
static u32 seqDiagnosticNoteStartFrame[256];
static uint64_t seqDiagnosticLifetimeFrameSum;
static u32 seqDiagnosticLifetimeCount;
static u32 seqDiagnosticLifetimeMin = 0xffffffff;
static u32 seqDiagnosticLifetimeMax;
static u32 seqDiagnosticProgramCount[128];
static u8 seqDiagnosticProgramByMidi[16];
static u32 seqDiagnosticMidiCount[16];
static u32 seqDiagnosticMaxVoicesCount[256];
static u32 seqDiagnosticMacroBits[2048];

static const char* SeqDiagnosticUserPath(const char* filename, char* path, size_t pathSize) {
...
static bool SeqDiagnosticMarkerEnabled(void) {
  char markerPath[1024];
  FILE* marker = fopen(SeqDiagnosticUserPath("audio_diagnostics.enable", markerPath, sizeof(markerPath)), "rb");
  if (marker == NULL) {
...
static bool IsDiagnosticGroup(u16 groupId) {
  const char* mode = getenv("PARTYBOARD_SEQ_DIAGNOSTICS");
  if ((mode != NULL && strcmp(mode, "all") == 0) || SeqDiagnosticMarkerEnabled()) {
    return true;
  }
  /* Front-end mode music plus the instruction-screen sequence. */
  return groupId == 93 || groupId == 98 || groupId == 112;
}

static FILE* GetSeqDiagnosticFile() {
  if (seqDiagnosticFile == NULL &&
      (getenv("PARTYBOARD_SEQ_DIAGNOSTICS") != NULL || SeqDiagnosticMarkerEnabled())) {
    const char* diagnosticMode = getenv("PARTYBOARD_SEQ_DIAGNOSTICS");
    if (diagnosticMode != NULL) {
      seqDiagnosticFile = fopen("sequence_diagnostic.log", "w");
    } else {
      char diagnosticPath[1024];
      seqDiagnosticFile = fopen(
          SeqDiagnosticUserPath("sequence_diagnostic.log", diagnosticPath, sizeof(diagnosticPath)), "w");
    }
  }
  return seqDiagnosticFile;
}
...
#if MUSY_TARGET == MUSY_TARGET_PC
static u32 CountNotes(NOTE* note) {
  u32 count = 0;
  while (note != NULL) {
    ++count;
    note = note->next;
...
static void LogNotePool(const char* reason) {
  FILE* file = GetSeqDiagnosticFile();
  if (file == NULL || cseq == NULL || !IsDiagnosticGroup(cseq->groupID)) {
    return;
  }
  fprintf(file, "NOTE_POOL %s frame=%u seq=%u group=%u free=%u used0=%u used1=%u keyoff=%u voicesMusic=%u voicesFx=%u starts=%u failures=%u noteLen=%llu noteVel=%llu noteKey=%llu lifeCount=%u lifeSum=%llu lifeMin=%u lifeMax=%u samples=%u sampleLag=%llu lagMax=%u missingSamples=%u sampleId=%llu sampleOff=%llu sampleLen=%llu sampleVol=%llu macroNote=%llu macroNoteHash=%llu finalNote=%llu finalNoteHash=%llu sampleInfo=%llu playFrq=%llu looped=%u nearEnd=%u pitchSum=%llu pitchHash=%llu pitchMin=%u pitchMax=%u fader=%.6f pause=%.6f active=%08x mute0=%08x mute1=%08x midiVol=",
          reason,
          salAudioDumpFrameCount, curSeqId, cseq->groupID, CountNotes(noteFree), CountNotes(cseq->noteUsed[0]),
          CountNotes(cseq->noteUsed[1]), CountNotes(cseq->noteKeyOff), voiceMusicRunning,
          voiceFxRunning, seqDiagnosticStarts, seqDiagnosticStartFailures,
          (unsigned long long)seqDiagnosticLengthSum, (unsigned long long)seqDiagnosticVelocitySum,
          (unsigned long long)seqDiagnosticKeySum, seqDiagnosticLifetimeCount,
          (unsigned long long)seqDiagnosticLifetimeFrameSum, seqDiagnosticLifetimeMin,
          seqDiagnosticLifetimeMax, startedSampleDiagnosticCount,
          (unsigned long long)startedSampleLagSum, startedSampleLagMax,
          missingSampleDiagnosticCount, (unsigned long long)startedSampleIdSum,
          (unsigned long long)startedSampleOffsetSum, (unsigned long long)startedSampleLengthSum,
          (unsigned long long)startedSampleVoiceVolumeSum,
          (unsigned long long)startedSampleCurrentNoteSum,
          (unsigned long long)startedSampleCurrentNoteHash,
          (unsigned long long)startedSampleFinalNoteSum,
          (unsigned long long)startedSampleFinalNoteHash,
          (unsigned long long)startedSampleInfoSum,
          (unsigned long long)startedSamplePlayFrqSum, startedSampleLoopedCount,
          startedSampleNearEndCount, (unsigned long long)salDiagPitchSum,
          (unsigned long long)salDiagPitchHash, salDiagPitchMin, salDiagPitchMax,
          synthMasterFader[cseq->defVGroup].volume,
          synthMasterFader[cseq->defVGroup].pauseVol, synthMasterFaderActiveFlags,
          cseq->trackMute[0], cseq->trackMute[1]);
  for (u32 channel = 0; channel < 16; ++channel) {
    fprintf(file, "%s%u", channel == 0 ? "" : ",", inpGetMidiCtrl(SND_MIDICTRL_VOLUME, channel, curSeqId));
  }
  fprintf(file, " pitchStages=%u,%llu,%llu,%llu,%llu,%llu,%llu events=%u,%u,%u,%u,%u,%u,%u midiPitch=",
          pitchStageCount, (unsigned long long)pitchStageBaseSum,
          (unsigned long long)pitchStageBendSum, (unsigned long long)pitchStageVibratoSum,
          (unsigned long long)pitchStagePortamentoSum, (unsigned long long)pitchStageAdsrSum,
          (unsigned long long)pitchStageFinalSum, eventRequestCount,
          eventAlreadyQueuedCount, eventHandledCount, eventStartCount,
          eventInactiveCount, eventBlockedCount, eventStaleCount);
  for (u32 channel = 0; channel < 16; ++channel) {
    fprintf(file, "%s%u", channel == 0 ? "" : ",",
            inpGetMidiCtrl(SND_MIDICTRL_PITCHBEND, channel, curSeqId));
  }
  fprintf(file, " midiPortamento=");
  for (u32 channel = 0; channel < 16; ++channel) {
    fprintf(file, "%s%u", channel == 0 ? "" : ",", inpGetMidiCtrl(65, channel, curSeqId));
  }
  fprintf(file, " midiDoppler=");
  for (u32 channel = 0; channel < 16; ++channel) {
    fprintf(file, "%s%u", channel == 0 ? "" : ",", inpGetMidiCtrl(SND_MIDICTRL_DOPPLER, channel, curSeqId));
  }
  fprintf(file, " midiPedal=");
  for (u32 channel = 0; channel < 16; ++channel) {
    fprintf(file, "%s%u", channel == 0 ? "" : ",", inpGetMidiCtrl(64, channel, curSeqId));
  }
  fprintf(file,
          " alloc=%u,%u,%u,%u,%u peakMusic=%u programs=",
          voiceDiagAllocationRequests, voiceDiagAllocationFree,
          voiceDiagAllocationReplacement, voiceDiagAllocationSameId,
          voiceDiagAllocationFailed, voiceDiagMusicPeak);
  for (u32 program = 0; program < 128; ++program) {
    if (seqDiagnosticProgramCount[program] != 0) {
      fprintf(file, "%u:%u,", program, seqDiagnosticProgramCount[program]);
    }
  }
  fprintf(file, " midi=");
  for (u32 channel = 0; channel < 16; ++channel) {
    if (seqDiagnosticMidiCount[channel] != 0) {
      fprintf(file, "%u:%u,", channel, seqDiagnosticMidiCount[channel]);
    }
  }
  fprintf(file, " maxVoices=");
  for (u32 limit = 0; limit < 256; ++limit) {
    if (seqDiagnosticMaxVoicesCount[limit] != 0) {
      fprintf(file, "%u:%u,", limit, seqDiagnosticMaxVoicesCount[limit]);
    }
  }
  u32 macroCount = 0;
  for (u32 word = 0; word < 2048; ++word) {
    u32 bits = seqDiagnosticMacroBits[word];
    while (bits != 0) {
      bits &= bits - 1;
      ++macroCount;
    }
  }
  fprintf(file, " macros=%u\n", macroCount);
  fflush(file);
  seqDiagnosticStarts = 0;
  seqDiagnosticStartFailures = 0;
  seqDiagnosticLengthSum = 0;
  seqDiagnosticVelocitySum = 0;
  seqDiagnosticKeySum = 0;
  seqDiagnosticLifetimeFrameSum = 0;
  seqDiagnosticLifetimeCount = 0;
  seqDiagnosticLifetimeMin = 0xffffffff;
  seqDiagnosticLifetimeMax = 0;
  memset(seqDiagnosticProgramCount, 0, sizeof(seqDiagnosticProgramCount));
  memset(seqDiagnosticMidiCount, 0, sizeof(seqDiagnosticMidiCount));
  memset(seqDiagnosticMaxVoicesCount, 0, sizeof(seqDiagnosticMaxVoicesCount));
  memset(seqDiagnosticMacroBits, 0, sizeof(seqDiagnosticMacroBits));
  voiceDiagAllocationRequests = 0;
  voiceDiagAllocationFree = 0;
  voiceDiagAllocationReplacement = 0;
  voiceDiagAllocationSameId = 0;
  voiceDiagAllocationFailed = 0;
  voiceDiagMusicPeak = voiceMusicRunning;
  startedSampleDiagnosticCount = 0;
  missingSampleDiagnosticCount = 0;
  startedSampleIdSum = 0;
  startedSampleOffsetSum = 0;
  startedSampleLengthSum = 0;
  startedSampleVoiceVolumeSum = 0;
  startedSampleLoopedCount = 0;
  startedSampleNearEndCount = 0;
  startedSampleLagSum = 0;
  startedSampleLagMax = 0;
  startedSampleCurrentNoteSum = 0;
  startedSampleCurrentNoteHash = 1469598103934665603ULL;
  startedSampleFinalNoteSum = 0;
  startedSampleFinalNoteHash = 1469598103934665603ULL;
  startedSampleInfoSum = 0;
  startedSamplePlayFrqSum = 0;
  salDiagPitchSum = 0;
  salDiagPitchHash = 1469598103934665603ULL;
  salDiagPitchMin = 0xffffffff;
  salDiagPitchMax = 0;
  pitchStageCount = 0;
  pitchStageBaseSum = 0;
  pitchStageBendSum = 0;
  pitchStageVibratoSum = 0;
  pitchStagePortamentoSum = 0;
  pitchStageAdsrSum = 0;
  pitchStageFinalSum = 0;
  eventRequestCount = 0;
  eventAlreadyQueuedCount = 0;
  eventHandledCount = 0;
  eventStartCount = 0;
  eventInactiveCount = 0;
  eventBlockedCount = 0;
  eventStaleCount = 0;
}
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
        if (IsDiagnosticGroup(cseq->groupID)) {
          const u32 lifetime = salAudioDumpFrameCount - seqDiagnosticNoteStartFrame[note - seqNote];
          seqDiagnosticLifetimeFrameSum += lifetime;
          ++seqDiagnosticLifetimeCount;
          if (lifetime < seqDiagnosticLifetimeMin)
            seqDiagnosticLifetimeMin = lifetime;
          if (lifetime > seqDiagnosticLifetimeMax)
            seqDiagnosticLifetimeMax = lifetime;
        }
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
    if (IsDiagnosticGroup(seq->groupID)) {
      seqDiagnosticProgramByMidi[midi] = prg;
    }
#endif
...
u32 seqStartPlay(PAGE* norm, PAGE* drum, MIDISETUP* midiSetup, u32* song, SND_PLAYPARA* para,
  ...
  if (IsDiagnosticGroup(sgid)) {
    ...
    seqDiagnosticLifetimeMax = 0;
    memset(seqDiagnosticProgramCount, 0, sizeof(seqDiagnosticProgramCount));
    memset(seqDiagnosticProgramByMidi, 0xff, sizeof(seqDiagnosticProgramByMidi));
    memset(seqDiagnosticMidiCount, 0, sizeof(seqDiagnosticMidiCount));
    memset(seqDiagnosticMaxVoicesCount, 0, sizeof(seqDiagnosticMaxVoicesCount));
    memset(seqDiagnosticMacroBits, 0, sizeof(seqDiagnosticMacroBits));
    voiceDiagAllocationRequests = 0;
  ...
  if (IsDiagnosticGroup(sgid) && GetSeqDiagnosticFile() != NULL) {
    fprintf(seqDiagnosticFile,
            "START frame=%u seq=%u sgid=%u info=%08x tTab=%u pTab=%u loop=%u sectioned=%u\n",
            salAudioDumpFrameCount, seqId, sgid, arr->info, arr->tTab, arr->pTab, arr->loopPoint[0],
            (arr->info & 0x80000000) != 0);
    for (i = 0; i < 64; ++i) {
      TENTRY* diagnosticEntry = nseq->track[i].base;
      u32 diagnosticCount = 0;
      if (diagnosticEntry == NULL) {
        continue;
      }
      while (diagnosticCount < 4096 && diagnosticEntry->pattern < 0xfffe) {
        ++diagnosticEntry;
        ++diagnosticCount;
      }
      fprintf(seqDiagnosticFile, "TRACK %d count=%u marker=%04x time=%u rewind=%u\n", i,
              diagnosticCount, diagnosticEntry->pattern, diagnosticEntry->time,
              *((u16*)&diagnosticEntry->transpose));
    }
    fflush(seqDiagnosticFile);
  }
#endif
...
void seqKillAllInstances() {
  ...
  /* seqStop moves the instance to seqFreeRoot and therefore overwrites its
   * next pointer. Save the active-list successor before stopping, otherwise
   * the traversal can jump into the free list and leave music sequences alive. */
  for (si = seqActiveRoot; si != NULL; si = nextSi) {
    nextSi = si->next;
    seqStop(si->publicId);
  }

  for (si = seqPausedRoot; si != NULL; si = nextSi) {
    nextSi = si->next;
    seqStop(si->publicId);
  }
}
...
#if MUSY_TARGET == MUSY_TARGET_PC
    if (IsDiagnosticGroup(cseq->groupID) && GetSeqDiagnosticFile() != NULL) {
      fprintf(seqDiagnosticFile,
              "PATTERN_EVENT frame=%u seq=%u group=%u loop=%u track=%u eventTime=%u "
              "sectionTime=%u pattern=%u midi=%u\n",
              salAudioDumpFrameCount, curSeqId, cseq->groupID,
              cseq->section[secIndex].loopCnt, event->trackId, event->time,
              cseq->section[secIndex].time[cseq->section[secIndex].timeIndex].high,
              tEntry->pattern,
              ARR_GET_TYPE(cseq->arrbase, cseq->arrbase->tmTab, u8*)[event->trackId]);
      fflush(seqDiagnosticFile);
    }
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
    if (IsDiagnosticGroup(cseq->groupID) && GetSeqDiagnosticFile() != NULL &&
        cseq->section[secIndex].loopCnt != 0 &&
        (event->trackId == 14 || event->trackId == 15 || event->trackId == 23)) {
      fprintf(seqDiagnosticFile,
              "LOOP_NOTE_EVENT frame=%u seq=%u group=%u loop=%u track=%u midi=%u "
              "eventTime=%u sectionTime=%u key=%u velocity=%u length=%u endTime=%u "
              "timeIndex=%u\n",
              salAudioDumpFrameCount, curSeqId, cseq->groupID,
              cseq->section[secIndex].loopCnt, event->trackId, midi, event->time,
              cseq->section[secIndex].time[cseq->section[secIndex].timeIndex].high,
              key, velocity, pe->length, event->time + pe->length,
              cseq->section[secIndex].timeIndex);
      fflush(seqDiagnosticFile);
    }
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
          if (IsDiagnosticGroup(cseq->groupID)) {
            seqDiagnosticNoteStartFrame[note - seqNote] = salAudioDumpFrameCount;
            ++seqDiagnosticStarts;
            seqDiagnosticLengthSum += pe->length;
            seqDiagnosticVelocitySum += velocity;
            seqDiagnosticKeySum += key;
            if (seqDiagnosticProgramByMidi[midi] < 128) {
              ++seqDiagnosticProgramCount[seqDiagnosticProgramByMidi[midi]];
            }
            ++seqDiagnosticMidiCount[midi];
            ++seqDiagnosticMaxVoicesCount[cseq->prgState[midi].maxVoices];
            seqDiagnosticMacroBits[macId >> 5] |= 1U << (macId & 31);
          }
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
            if (IsDiagnosticGroup(cseq->groupID)) {
              ++seqDiagnosticStartFailures;
              FILE* file = GetSeqDiagnosticFile();
              if (file != NULL) {
                u16 layerCount = 0;
                LAYER* layer = NULL;
                fprintf(file,
                        "START_FAILURE frame=%u seq=%u group=%u track=%u time=%u section=%u "
                        "midi=%u program=%u macro=%u macroType=%04x key=%u velocity=%u "
                        "length=%u priority=%u maxVoices=%u volGroup=%u studio=%u\n",
                        salAudioDumpFrameCount, curSeqId, cseq->groupID, event->trackId,
                        event->time, secIndex, midi, seqDiagnosticProgramByMidi[midi], macId,
                        macId & 0xc000, key, velocity, pe->length,
                        cseq->prgState[midi].priority, cseq->prgState[midi].maxVoices,
                        cseq->trackVolGroup[event->trackId], cseq->defStudio);
                if ((macId & 0xc000) == 0x8000) {
                  layer = dataGetLayer(macId, &layerCount);
                  fprintf(file, "LAYER_FAILURE id=%u found=%u entries=%u", macId,
                          layer != NULL, layerCount);
                  for (u16 layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
                    fprintf(file, " [%u:id=%u keys=%u-%u transpose=%d volume=%u priority=%d pan=%u]",
                            layerIndex, layer[layerIndex].id, layer[layerIndex].keyLow,
                            layer[layerIndex].keyHigh, layer[layerIndex].transpose,
                            layer[layerIndex].volume, layer[layerIndex].prioOffset,
                            layer[layerIndex].panning);
                  }
                  fprintf(file, "\n");
                }
                fflush(file);
...
#if MUSY_TARGET == MUSY_TARGET_PC
    if (IsDiagnosticGroup(cseq->groupID) && GetSeqDiagnosticFile() != NULL) {
      fprintf(seqDiagnosticFile, "LOOP_EVENT seq=%u track=%u time=%u\n", curSeqId,
              event->trackId, event->time);
      fflush(seqDiagnosticFile);
      if (event->trackId == 11) {
        LogNotePool("LOOP");
      }
...
#if MUSY_TARGET == MUSY_TARGET_PC
      if (IsDiagnosticGroup(cseq->groupID) && GetSeqDiagnosticFile() != NULL) {
        fprintf(seqDiagnosticFile, "END seq=%u time=%u loops=%u\n", curSeqId,
                cseq->section[0].time[cseq->section[0].timeIndex].high,
                cseq->section[0].loopCnt);
        fflush(seqDiagnosticFile);
      }
#endif
#if MUSY_VERSION >= MUSY_VERSION_CHECK(2, 0, 1)
