...
u32 voiceDiagAllocationRequests = 0;
u32 voiceDiagAllocationFree = 0;
u32 voiceDiagAllocationReplacement = 0;
u32 voiceDiagAllocationSameId = 0;
u32 voiceDiagAllocationFailed = 0;
u32 voiceDiagMusicPeak = 0;
#endif
...
u32 voiceAllocate(u8 priority, u8 maxVoices,
...
#if MUSY_TARGET == MUSY_TARGET_PC
  ++voiceDiagAllocationRequests;
  if (voice == -1) {
    ++voiceDiagAllocationFailed;
  } else if (voiceList[voice].user == 1) {
    ++voiceDiagAllocationFree;
  } else {
    ++voiceDiagAllocationReplacement;
    if (synthVoice[voice].allocId == allocId) {
      ++voiceDiagAllocationSameId;
    }
  }
#endif
  u32 result = voiceAllocateDo(voice, fxFlag);
#if MUSY_TARGET == MUSY_TARGET_PC
  if (!fxFlag && result != SND_ID_ERROR && voiceMusicRunning > voiceDiagMusicPeak) {
  ...
  _update:
    ...
    if (fxFlag != FALSE) {
      ...
      ++voiceMusicRunning;
#if MUSY_TARGET == MUSY_TARGET_PC
      if (voiceMusicRunning > voiceDiagMusicPeak) {
        voiceDiagMusicPeak = voiceMusicRunning;
      }
#endif
    }
