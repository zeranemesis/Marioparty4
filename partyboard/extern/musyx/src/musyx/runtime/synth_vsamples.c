...
void sndVirtualSampleARAMUpdate(SND_INSTID instID, void* base, u32 off1, u32 len1, u32 off2,
  ...
  for (i = 0; i < vs.numBuffers; ++i) {
    ...
    if (vs.streamBuffer[i].smpType == 5) {
      ...
      /* The ADPCM predictor/scale is the first byte in the stream block.
       * A u32 >> 24 reads the wrong byte on little-endian hosts. */
      hwSetStreamLoopPS(vs.streamBuffer[i].voice, *(u8*)base);
#endif
    }
    break;
  }
