...
void salReconnectVoice(DSPvoice* dsp_vptr, u8 studio) {
  if (dsp_vptr->state != 0) {
#if MUSY_TARGET == MUSY_TARGET_PC
    /* The GameCube command-list path deposits a running voice's final DSP
     * contribution in the old studio through alienVoiceRoot.  The PC mixer
     * renders directly, so queue that contribution before changing studio. */
    if (dsp_vptr->state == 2 && dsp_vptr->studio != studio) {
      salPCDepopVoice(dsp_vptr);
    }
#endif
