...
#if MUSY_TARGET == MUSY_TARGET_PC
u32 pitchStageCount;
uint64_t pitchStageBaseSum;
uint64_t pitchStageBendSum;
uint64_t pitchStageVibratoSum;
uint64_t pitchStagePortamentoSum;
uint64_t pitchStageAdsrSum;
uint64_t pitchStageFinalSum;
u32 eventRequestCount;
u32 eventAlreadyQueuedCount;
u32 eventHandledCount;
u32 eventStartCount;
u32 eventInactiveCount;
u32 eventBlockedCount;
u32 eventStaleCount;
#endif
...
static u32 StartLayer(u16 layerID, s16 prio, u8 maxVoices,
  ...
  mKey = key & 0x7f;
  for (; n != 0; --n, l++) {
    /* Some MP4 layer tables stop the final range at MIDI 95 even though the
     * sequence legitimately emits keys above it.  The final layer is the
     * high-register fallback; rejecting it drops the note at every loop. */
    if (l->id == 0xffff || l->keyLow > mKey || (l->keyHigh < mKey && n != 1)) {
      continue;
    }
...
static u32 StartKeymap(u16 keymapID, s16 prio, u8 maxVoices,
  ...
  if ((keymap = dataGetKeymap(keymapID)) != NULL) {
    o = key & 0x7f;
    /* A few MP4 keymaps leave the top MIDI slots empty although the sequence
     * emits those keys.  Reuse the nearest lower valid mapping so the note is
     * still played with the intended instrument instead of being dropped. */
    while (o > 0 &&
           (keymap[o].id == 0xffff || (keymap[o].id & 0xc000) == 0x4000)) {
      --o;
    }
    if (keymap[o].id != 0xffff && (keymap[o].id & 0xc000) != 0x4000) {
      if ((keymap[o].panning & 0x80) == 0) {
        p = (keymap[o].panning - 0x40);
        p += panning;
        if (p < 0) {
          panning = 0;
        } else if (p > 0x7f) {
...
static void LowPrecisionHandler(u32 i) {
  ...
  s32 voff;         // r22
#if MUSY_TARGET == MUSY_TARGET_PC
  u8 diagnosticInitialPitch;
#endif

  sv = &synthVoice[i];
#if MUSY_TARGET == MUSY_TARGET_PC
  diagnosticInitialPitch = (sv->cFlags & 0x20) != 0;
#endif
  /* Keep a freshly initialized sample eligible for its first pitch update,
   * even when its one-shot macro has already released the synth slot. */
#if MUSY_TARGET == MUSY_TARGET_PC
  if (!hwIsActive(i) && sv->addr == NULL && (sv->cFlags & 0x20) == 0) {
#else
...
#if MUSY_TARGET == MUSY_TARGET_PC
  if (diagnosticInitialPitch) {
    ++pitchStageCount;
    pitchStageBaseSum += ccents;
  }
#endif
  if ((sv->cFlags &
#if MUSY_VERSION >= MUSY_VERSION_CHECK(1, 5, 4)
...
#if MUSY_TARGET == MUSY_TARGET_PC
  if (diagnosticInitialPitch)
    pitchStagePortamentoSum += ccents;
#endif
  if ((sv->cFlags & 0x20000000000) != 0) {
...
static void ZeroOffsetHandler(u32 i) {
  ...
  sv = &synthVoice[i];
  /* Keep the initial volume/pan update paired with a pending sample start,
   * even when its one-shot macro has already released the synth slot. */
#if MUSY_TARGET == MUSY_TARGET_PC
  if (!hwIsActive(i) && sv->addr == NULL && (sv->cFlags & 0x20) == 0) {
#else
...
static void EventHandler(u32 i) {
  ...
  ++eventHandledCount;
#endif
  /* Some Mario Party 4 macros end immediately after Start Sample. On PC the
   * host voice has not been activated yet, but cFlags still carries the
   * pending start prepared by hwInitSamplePlayback. Let the initial pitch
   * update run before EventHandler activates that sample. */
#if MUSY_TARGET == MUSY_TARGET_PC
  if (!hwIsActive(i) && sv->addr == NULL && (sv->cFlags & 0x20) == 0) {
#else
  if (!hwIsActive(i) && sv->addr == NULL) {
...
static void synthAddJob(SYNTH_VOICE* svoice, SYNTH_JOBTYPE jobType, u32 deltaTime) {
  ...
  case SYNTH_JOBTYPE_EVENT:
    ...
    if (newJq->jobTabIndex != 0xff) {
      ...
      /* Controller changes can leave an event queued for a later job-table
       * slot.  If this voice is recycled for a new sample before that slot is
       * reached, keeping the old schedule delays the sample start by as much
       * as 31 synth ticks.  Move the one per-voice event to the requested
       * slot, just like the low-precision and zero-offset jobs above. */
      if (newJq->next != NULL) {
        newJq->next->prev = newJq->prev;
      }
      if (newJq->prev != NULL) {
        ...
        synthJobTable[newJq->jobTabIndex].event = newJq->next;
      }
#if MUSY_TARGET == MUSY_TARGET_PC
      ++eventStaleCount;
#endif
      newJq->jobTabIndex = 0xff;
      newJq->next = NULL;
      newJq->prev = NULL;
    }
    root = &jobTab->event;
...
void synthKeyStateUpdate(SYNTH_VOICE* svoice) {
  /* Keep the original MusyX scheduling rule: an ended one-shot macro may no
   * longer own a public voice id while its hardware sample is still active.
   * Testing svoice->id here would turn the released sentinel (0xFFFFFFFF)
   * into voice index 255 and can suppress updates for still-audible samples. */
#if MUSY_TARGET == MUSY_TARGET_PC
  ++eventRequestCount;
  eventAlreadyQueuedCount += svoice->eventJob.jobTabIndex != 0xff;
#endif
  synthAddJob(svoice, SYNTH_JOBTYPE_EVENT, 0);
}
...
u32 synthFXStart(u16 fid,
  ...
  if ((fx = dataGetFX(fid)) != NULL) {
    ...
    if (v == 0xFFFFFFFF) {
      fprintf(stderr,
              "[MusyX] FX allocation failed fx=%u macro=%u priority=%u maxVoices=%u vGroup=%u\n",
              fid, fx->macro, fx->priority, fx->maxVoices, fx->vGroup);
    }
#endif
  }
#if MUSY_TARGET == MUSY_TARGET_PC
  else {
    fprintf(stderr, "[MusyX] FX table entry missing fx=%u\n", fid);
  }
#endif
