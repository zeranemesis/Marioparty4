...
u32 missingSampleDiagnosticCount;
u32 startedSampleDiagnosticCount;
uint64_t startedSampleIdSum;
uint64_t startedSampleOffsetSum;
uint64_t startedSampleLengthSum;
uint64_t startedSampleVoiceVolumeSum;
u32 startedSampleLoopedCount;
u32 startedSampleNearEndCount;
u32 sampleMacroStartFrame[SYNTH_MAX_VOICES];
uint64_t startedSampleLagSum;
u32 startedSampleLagMax;
uint64_t startedSampleCurrentNoteSum;
uint64_t startedSampleCurrentNoteHash = 1469598103934665603ULL;
uint64_t startedSampleFinalNoteSum;
uint64_t startedSampleFinalNoteHash = 1469598103934665603ULL;
uint64_t startedSampleInfoSum;
uint64_t startedSamplePlayFrqSum;
static u32 targetMacroTraceCount;

static bool traceTargetTrack(u16 track) {
  return getenv("PARTYBOARD_RENDER_DIAGNOSTICS") != NULL &&
         (track == 14 || track == 15 || track == 23) && targetMacroTraceCount < 8192;
}
#endif

/* MusyX curve payloads are always serialized little-endian (unlike most of
 * the GameCube project structures).  The original PowerPC runtime therefore
 * byte-swaps 16-bit curve fields at the point of use.  On the little-endian
 * PC host those values are already native and swapping them again corrupts
 * ADSR attack/decay/sustain/release timings. */
static u16 readCurveU16(u16 value) {
#if MUSY_TARGET == MUSY_TARGET_PC
  return value;
#else
...
#include "synth_wait.h"

static u32 mcmdEndOfMacro(SYNTH_VOICE* svoice) {
#if MUSY_TARGET == MUSY_TARGET_PC
  if (traceTargetTrack(svoice->track)) {
    fprintf(stderr,
            "[MusyX] TARGET_MAC_END frame=%u voice=%u track=%u macro=%u sampleInfo=%u "
            "active=%u flags=%llx state=%u\n",
            salAudioDumpFrameCount, svoice->id & 0xff, svoice->track, svoice->macroId,
            svoice->sInfo, hwIsActive(svoice->id & 0xff),
            (unsigned long long)svoice->cFlags, svoice->macState);
    ++targetMacroTraceCount;
  }
#endif
...
static void mcmdStartSample(SYNTH_VOICE* svoice, MSTEP* cstep) {
  ...
  if (dataGetSample(smp, &newsmp) != 0) {
    ...
    if (getenv("PARTYBOARD_RENDER_DIAGNOSTICS") != NULL &&
        (missingSampleDiagnosticCount < 64 || (missingSampleDiagnosticCount % 256) == 0)) {
      fprintf(stderr, "[MusyX] missing sample frame=%u sample=%u count=%u\n",
              salAudioDumpFrameCount, smp, missingSampleDiagnosticCount);
    }
    ++missingSampleDiagnosticCount;
#endif
    return;
  }
#if MUSY_TARGET == MUSY_TARGET_PC
  if (traceTargetTrack(svoice->track)) {
    fprintf(stderr,
            "[MusyX] TARGET_SAMPLE frame=%u voice=%u track=%u macro=%u sample=%u "
            "note=%u volume=%u flags=%llx\n",
            salAudioDumpFrameCount, svoice->id & 0xff, svoice->track, svoice->macroId, smp,
            svoice->curNote, (u8)(svoice->volume >> 16),
            (unsigned long long)svoice->cFlags);
    ++targetMacroTraceCount;
  }
  ++startedSampleDiagnosticCount;
  const u32 sampleLag = salAudioDumpFrameCount - sampleMacroStartFrame[svoice->id & 0xff];
  startedSampleLagSum += sampleLag;
  if (sampleLag > startedSampleLagMax)
    startedSampleLagMax = sampleLag;
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
  startedSampleIdSum += smp;
  startedSampleOffsetSum += newsmp.offset;
  startedSampleLengthSum += newsmp.length;
  startedSampleVoiceVolumeSum += (u32)svoice->volume;
  startedSampleCurrentNoteSum += svoice->curNote;
  startedSampleCurrentNoteHash = (startedSampleCurrentNoteHash ^ svoice->curNote) * 1099511628211ULL;
  startedSampleLoopedCount += newsmp.loopLength != 0;
  startedSampleNearEndCount += newsmp.length != 0 && newsmp.offset >= newsmp.length / 2;
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
  startedSampleFinalNoteSum += svoice->curNote;
  startedSampleFinalNoteHash = (startedSampleFinalNoteHash ^ svoice->curNote) * 1099511628211ULL;
  startedSampleInfoSum += svoice->sInfo;
  startedSamplePlayFrqSum += svoice->playFrq;
#endif
  svoice->cFlags |= 0x20;
...
static void mcmdSetADSR(SYNTH_VOICE* svoice, MSTEP* cstep) {
  ...
  if ((adsr_ptr = dataGetCurve(cstep->para[0] >> 8)) != NULL) {
    if (!(u8)(cstep->para[0] >> 24)) {
      adsr.data.linear.atime = readCurveU16(adsr_ptr->data.linear.atime);
      adsr.data.linear.dtime = readCurveU16(adsr_ptr->data.linear.dtime);
      adsr.data.linear.slevel = readCurveU16(adsr_ptr->data.linear.slevel);
      adsr.data.linear.rtime = readCurveU16(adsr_ptr->data.linear.rtime);
      hwSetADSR(svoice->id & 0xFF, &adsr, FALSE);
    } else {
      sScale = dspDLSVolTab[readCurveU16(adsr_ptr->data.dls.slevel) >> 5];
      adsr.data.dls.atime =
      ...
      adsr.data.dls.rtime = readCurveU16(adsr_ptr->data.dls.rtime);
      ascale =
...
static void mcmdSetPitchADSR(SYNTH_VOICE* svoice, MSTEP* cstep) {
  ...
  adsr.data.dls.slevel = readCurveU16(adsr_ptr->data.dls.slevel);
  adsr.data.dls.rtime = readCurveU16(adsr_ptr->data.dls.rtime);
  ascale =
      (((u8*)&adsr_ptr->data.dls.ascale)[0] << 0) | (((u8*)&adsr_ptr->data.dls.ascale)[1] << 8) |
      (((u8*)&adsr_ptr->data.dls.ascale)[2] << 16) | (((u8*)&adsr_ptr->data.dls.ascale)[3] << 24);
...
#if MUSY_TARGET == MUSY_TARGET_PC
      sampleMacroStartFrame[voice] = salAudioDumpFrameCount;
#endif
      vidRemoveVoiceReferences(svoice);
...
#if MUSY_TARGET == MUSY_TARGET_PC
      if (traceTargetTrack(trackid)) {
        fprintf(stderr,
                "[MusyX] TARGET_MAC_START frame=%u voice=%u track=%u macro=%u key=%u "
                "alloc=%u maxVoices=%u flags=%llx\n",
                salAudioDumpFrameCount, voice, trackid, macid, key, allocId, maxVoices,
                (unsigned long long)svoice->cFlags);
        ++targetMacroTraceCount;
      }
#endif
...
#if MUSY_TARGET == MUSY_TARGET_PC
    else if (traceTargetTrack(trackid)) {
      fprintf(stderr,
              "[MusyX] TARGET_MAC_ALLOC_FAIL frame=%u track=%u macro=%u key=%u alloc=%u "
              "maxVoices=%u\n",
              salAudioDumpFrameCount, trackid, macid, key & 0x7f, allocId, maxVoices);
      ++targetMacroTraceCount;
    }
#endif
  }
