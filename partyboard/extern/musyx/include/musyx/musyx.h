...
/*
 * Some games keep sample directories in separate groups.  The PC port can
 * ask its host to supply one when a group refers to an otherwise unavailable
 * sample.  The host retains ownership of `samples` after a successful call.
 */
typedef bool (*SND_SAMPLE_RESOLVER)(u16 sampleId);
void sndSetSampleResolver(SND_SAMPLE_RESOLVER resolver);
